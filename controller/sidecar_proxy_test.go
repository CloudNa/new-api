...
func TestCPAManagerPlusAPILiteralsUseBridgePrefix(t *testing.T) {
	...
	got := string(replaceSidecarAbsolutePaths(body, target))
	want := `const MANAGEMENT_API_PREFIX="/cpa/v0/management";const API_ENDPOINTS={CONFIG:"/cpa/config",AUTH_FILES:"/cpa/auth-files",INFO:"/cpa/usage-service/info",STATUS:"/cpa/status"};const ROUTES={CONFIG:"#/config",AUTH_FILES:"#/auth-files",LOGS:"#/logs"};`
	if got != want {
		t.Fatalf("replaceSidecarAbsolutePaths() = %q, want %q", got, want)
	}
}
